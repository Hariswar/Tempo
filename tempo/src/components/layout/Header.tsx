...
import {
  ...
  LayoutGrid, Columns, Calendar, Sparkles,
  CheckCircle2
} from 'lucide-react'
...
export default function Header() {
  ...
  function navigate(dir: 1 | -1) {
    if (viewMode === 'week') {
      ...
      // month: skip 4 weeks approx
      setSelectedDate((dir === 1 ? addWeeks : subWeeks)(date, 4).toISOString())
    }
  }
  ...
  return (
    <header
      ...
      style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#0a0a16' }}
    >
      {/* Navigation */}
      <div className="flex items-center gap-1">
        ...
        <button
          onClick={goToToday}
          className="px-2.5 h-7 text-xs font-medium rounded-lg transition-colors hover:bg-white/5 text-text-secondary hover:text-text-primary"
        >
          Today
        </button>
      </div>
      ...
      {/* Date label */}
      <h1 className="text-sm font-semibold text-text-primary min-w-[140px]">{headerLabel}</h1>
      ...
      <div className="flex-1 max-w-xs">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 h-7 text-xs rounded-lg text-text-primary placeholder:text-text-muted outline-none transition-colors"
            style={{
      ...
      <button
        ...
        style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.3), rgba(79,70,229,0.2))', border: '1px solid rgba(124,58,237,0.3)', color: '#a78bfa' }}
      >
        <Sparkles size={12} />
        <span>AI</span>
      </button>
      ...
      <div className="relative">
        <button
          ...
          className="w-8 h-8 flex items-center justify-center rounded-lg transition-colors hover:bg-white/5 relative"
        >
